...
test('it shows the title in the banner', () => {
	setup();
	// We expect that the title 'Hi, Iam Lhakpa Tenji Sherpa' is in the banner component
	expect(screen.getByText(/Hi, Iam Lhakpa Tenji Sherpa/i)).toBeInTheDocument();
});
...
test('can download cv when clicked on download cv button', async () => {
	...
	// expect(downloadLink.download).toEqual('Lhakpa-Tenji-Sherpa-Resume.pdf');
	// expect(downloadLink.href).toEqual('/files/Lhakpa-Tenji-Sherpa-Resume.pdf');
